document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelectorAll(".reptile-fact").forEach((fact) => {
    ...
    document.querySelectorAll(".reptile-text p:first-of-type").forEach((p) => {
    ...
    const cards = Array.from(document.querySelectorAll(".reptiles-grid .reptile-card"));
    ...
    if (cards.length && numbersEl && prevPageBtn && nextPageBtn) {
        ...
        function goToPage(page, scroll) {
            ...
            if (infoEl) {
                infoEl.textContent = `Mostrando ${start}–${end} de ${cards.length} reptiles`;
            }

            if (scroll) {
                const section = document.querySelector(".reptiles-catalog");
                if (section) {
                    window.scrollTo({
