document.addEventListener("DOMContentLoaded", () => {
    ...
    sliders.forEach((sliderContainer) => {
        const slides = sliderContainer.querySelectorAll(".amphibian-slider img");
    ...
    if (cards.length && numbersEl && prevPageBtn && nextPageBtn) {
        ...
        function goToPage(page, scroll) {
            ...
            if (infoEl) {
                infoEl.textContent = `Mostrando ${start}–${end} de ${cards.length} anfibios`;
            }

            if (scroll) {
                const section = document.querySelector(".amphibians-catalog");
                if (section) {
                    window.scrollTo({
